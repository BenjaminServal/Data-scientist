...
def size_handle(journalists)
 puts journalists.sort{|a, b| a.length <=> b.length}
end

 #Quelle est la position dans l'array de la personne @epenser ?
 def pos_handle(journalists)
 ...
 def size_count(journalists)
    i = 0
    journalists.max{|a, b| a.length <=> b.length} #combien de caractere max il y a (33)
    while (i <= 33)
    ...
    puts "A quelle question, entre 1 et 8 veux tu que je reponde?"
    print ">"
 ...
 #min_handle(journalists)
 #handle_5(journalists)
 #up_handle(journalists)
 #tri_handle(journalists)
#size_handle(journalists)
#pos_handle(journalists)
#nb_handle(journalists)
#size_count(journalists)